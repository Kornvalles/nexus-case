...
const _rootStore = RootStoreModel.create({
  taskStore: {
    tasks: [
      { id: uuidv4(), title: "Do the dishes", completed: null },
      { id: uuidv4(), title: "Do the laundry", completed: null },
      {
        id: uuidv4(),
        title: "Mow the lawn",
        completed: new Date().toDateString(),
      },
    ],
  },
});
...
export const useInitialRootStore = (callback: () => void | Promise<void>) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { /* restoredState, */ unsubscribe } = await setupRootStore(
        rootStore
      );
